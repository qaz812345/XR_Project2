...
Shader "Paint in 3D/Alpha"
{
   Properties
   {
	...
	[Toggle(_USE_UV2)] _UseUV2("	Use Second UV", Float) = 1
...
#   define UNITY_LOOP
	...
	{
		v.texcoord1 = lerp(v.texcoord0, v.texcoord1, _UseUV2);
	...
	void Ext_SurfaceFunction0(inout Surface o, ShaderData d)
	{
		...
		float4 gloss   = SAMPLE_TEXTURE2D(_MetallicGlossMap, sampler_MainTex, d.texcoord0);
		float4 bump    = SAMPLE_TEXTURE2D(_BumpMap, sampler_MainTex, d.texcoord0);
		float4 glow    = SAMPLE_TEXTURE2D(_EmissionMap, sampler_MainTex, d.texcoord0);
	#if !_HAS_ALPHA_BLEND
